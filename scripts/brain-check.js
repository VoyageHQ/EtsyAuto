...
if (llm.provider === 'offline') {
  ...
  console.log(dim('    Install Ollama from https://ollama.com, then pick one:'));
  console.log(dim('      ollama pull hermes3        ~5GB. Tuned for structured output,'));
  console.log(dim('                                 which is most of what this project asks for.'));
  console.log(dim('      ollama pull qwen2.5:14b    ~9GB. Better writing, needs more memory.'));
  console.log(dim('      ollama pull llama3.1       ~5GB. The safe default.'));
  console.log(dim('    and in .env:'));
  console.log('      LLM_PROVIDER=openai');
  console.log('      LLM_BASE_URL=http://localhost:11434/v1');
  console.log('      LLM_MODEL=hermes3\n');
  console.log(dim('    Rough guide: 8GB of RAM runs an 8B model, 16GB runs a 14B, and'));
  console.log(dim('    anything with an Apple M-series chip or a modern GPU is comfortable.'));
  console.log(dim('    A slow answer is fine here — the agents work in the background.\n'));
